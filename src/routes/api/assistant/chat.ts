/**
 * Admin assistant ("Eva") chat endpoint.
 *
 * Authenticated: bearer token + admin role (developer / owner / manager).
 * Streams the model answer as SSE and persists both sides of the exchange
...
function gatewayErrorMessage(status: number, lang: AssistantLang, upstream: string) {
  ...
  if (status === 429)
    return en
      ...
      : "Asistentė šiuo metu užimta, pabandykite po akimirkos.";
  if (status === 402)
    return en
      ? "AI credits are exhausted. The workspace owner needs to top up credits."
      : "Baigėsi AI kreditai. Darbo srities savininkui reikia jų papildyti.";
  if (status === 403)
    return en
      ? "The AI assistant is blocked by workspace policy."
      : "AI asistentė užblokuota darbo srities nustatymais.";
  console.error("[assistant] gateway", status, upstream.slice(0, 300));
  return en ? "The assistant is temporarily unavailable." : "Asistentė laikinai nepasiekiama.";
}
...
async function handlePost(request: Request) {
  ...
  const history = ((historyRows ?? []).reverse() as { role: string; content: string }[]).map(
    (m) => ({ role: m.role === "assistant" ? "assistant" : "user", content: m.content }),
  );

  const ctx = await import("@/lib/assistant-context.server");
  const { buildBusinessSummary } = await import("@/lib/assistant-analytics.server");
  const [settings, unitsSummary, businessSummary] = await Promise.all([
    ctx.loadSettingsForAssistant(supabase),
    ctx.buildUnitsSummary(supabase, lang),
    buildBusinessSummary(supabase, lang).catch((e) => {
  ...
  const system = buildSystemPrompt({
    lang,
    brandName: settings.displayName?.trim() || (lang === "en" ? "the platform" : "sistemos"),
    settingsKnowledge: ctx.buildSettingsKnowledge(lang, settings),
    unitsSummary,
    businessSummary,
